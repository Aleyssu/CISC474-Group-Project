...
class pinfo:
    ...
    def init(H, W):
        pinfo.prev_agent_pos = None
        pinfo.path_of_least_resistance = None
        pinfo.prev_path_of_least_resistance = None
        pinfo.enemy_positions = []
        pinfo.enemy_orientations = dict()
        pinfo.unknown_enemies = set()
        pinfo.unknown_enemy_possible_orientations = dict()
        pinfo.enemy_wall_map = None
        pinfo.possible_dangerous_cells = set()
        pinfo.trap_cells = set()
        pinfo.trap_cells_prev = set()
        pinfo.bfs_cell_coords = deque()
        pinfo.following_bfs = False
        pinfo.just_started_following_bfs = False
        pinfo.enemy_wall_danger_map = dict()
        pinfo.enemy_orientations_computed = False
        pinfo.env_orientation = 0
        pinfo.push_traps = False
        pinfo.push_danger = False
        pinfo.is_dying = False
